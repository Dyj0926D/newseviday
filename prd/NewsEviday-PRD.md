...
| 文档版本   | v1.3.0                                                               |
| 文档状态   | P7 工程与发布完成；P8 首页视觉升级完成，Logo 方向待确认               |
| 更新日期   | 2026-08-03                                                           |
...
Logo 设计处于候选方向确认阶段。用户选定前继续使用现有文字标识；选定后再制作可确定性复现的 SVG、favicon 和深浅色变体，避免将概念图直接作为生产标识。
...
1. 完成已确认的首页视觉升级并通过 PC、移动端、减少动态和性能回归；
2. 确认 Logo 方向，再制作并接入生产 SVG、favicon 和深浅色变体；
3. 免费精简公开网址，并同步 Turnstile 允许的 hostname；
4. 按[AI 一周试运行评估](../docs/AI一周试运行评估.md)补齐批处理调用上限、GitHub Actions 配置和安全上限；
5. 完成前述安全配置后，再开始一周受控试运行；
6. 在生产语料和人工复核黄金集上重新运行检索与引用质量评测，动态趋势生成继续关闭；
7. 稳定大陆直连、自定义域名和备案延期处理；
8. 第二设备迁移演练延期，后续在个人设备按迁移文档执行；
