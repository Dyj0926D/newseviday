...
|---|---|---|---|---|---|
| `/` | 首页 / 情报流 | 核心版 | 全部动态、为你推荐、搜索和筛选 | Logo、产品介绍 CTA、直接访问 | 文章、简报、问答、画像 |
| `/article/:id` | 文章详情 | 核心版 | 中文整理、原始信息、证据、相关内容和当前文章追问 | 信息流、简报、RAG 引用 | 原文、相关文章、问答 |
| `/ask` | 情报问答 | 核心版 | 最近 30 天跨文章 RAG | 顶部导航、产品介绍 | 引用证据、文章、原文 |
| `/brief` | 7 日趋势简报 | 核心版 | 趋势、变化依据、来源和观察问题 | 顶部导航、首页信号概览 | 文章、原文、问答 |
| `/profile` | 我的画像 | 核心版 | 输入、AI 增强、确认、编辑、导入和导出 | “定制我的关注”、头像菜单 | 为你推荐、首页 |
| `/eval` | Eval | 核心版 | 指标、测试集、版本、门槛和失败案例 | 产品介绍、页脚、问答 Trace | 问答、产品介绍 |
| `/status` | 数据状态 | 核心版 | 更新时间、来源状态、AI 状态和已知问题 | 产品介绍、页脚、历史快照提示 | 首页、产品介绍 |
| `/product` | 产品介绍 | 核心版 | 产品思路、能力、架构、RAG、Eval、成本和边界 | 顶部导航、页脚 | 首页、问答、Eval、GitHub |
...
| 情报流 | `/?view=all` | 默认视图 |
| 为你推荐 | `/?view=for-you` | 无画像时展示通用流和画像引导，不强制创建画像 |
| 趋势简报 | `/brief` | 访客只查看公共简报，不直接触发生成 |
| 情报问答 | `/ask` | RAG 关闭时仍可查看页面和暂停说明 |
| 产品介绍 | `/product` | 同时服务普通用户和面试官 |
| 搜索 | `/?q={keyword}` | 从任意页面提交后进入首页搜索结果状态 |
| 定制我的关注 / 头像菜单 | `/profile` | 未设置画像时显示引导；已设置时显示编辑入口 |

Eval、数据状态、GitHub 和 PRD 不占用顶部主导航，通过产品介绍页、页脚和相关状态入口访问。
...
- 清空全部筛选回到 `/?view=all` 或 `/?view=for-you`；
...
| 文章详情 | `design/reference/视觉基准/02-文章详情-PC-reference-v1.png` |
| 情报问答 | `design/reference/视觉基准/03-情报问答-PC-reference-v1.png` |
...
主要入口：Logo、顶部“情报流/为你推荐”、产品介绍页“体验情报流”、站外分享链接。
...
主要去向：文章详情、7 日趋势简报、情报问答、我的画像、原始来源。

页面信息来源：

| 内容 | 来源 |
|---|---|
| 信息流、标题、摘要、标签 | 静态 `feed.json` 快照 |
...
该能力嵌入 `/article/:id`，不是独立页面。用户提交后停留在文章详情，并可以点击引用定位右侧证据或打开原文。
...
主要入口：产品介绍、页脚、情报问答“查看完整 Trace”。

主要去向：情报问答、失败案例对应文章、产品介绍技术章节。
...
主要入口：页脚、产品介绍、首页历史快照提示。

主要去向：首页、产品介绍、趋势简报。

页面信息来源：`status.json`、`PipelineRun`、`RuntimeConfig` 和脱敏后的来源运行结果。
...
主要去向：情报流、情报问答、Eval、数据状态、GitHub、PRD。

页面信息来源：产品文案和架构来自版本化仓库文档；界面预览来自实际产品截图；Eval 结果来自最新 `EvalRun`；状态来自 `status.json`；GitHub、PRD 和实施进度链接来自站点配置。

页面状态：

- 产品尚未完整上线：允许展示目标能力，但必须标记“规划中/目标值”；
- Eval 尚未运行：展示评测方法和目标门槛；
- 外部链接不可用：不阻塞页面阅读，提供复制地址；
...
- [ ] 从画像页返回首页时切换到 `/?view=for-you`，并展示推荐原因。
- [ ] 不支持账号同步，不制造已登录假象。
...
|---|---|---|
| 顶部 Logo | 首页全部动态 | 首页内点击回到顶部；跨页点击进入 `/?view=all` |
| 顶部“为你推荐” | 首页推荐视图 | 使用 `/?view=for-you`；无画像时显示引导 |
| 顶部搜索提交 | 首页搜索结果 | 使用 `q` 参数；保留其他合法筛选 |
| 信息流条目 | 文章详情 | 返回后恢复首页 URL、页码和滚动位置 |
| 信息流“查看原文” | 外部来源 | 新标签页打开；原页面状态不变 |
| 首页“查看趋势简报” | `/brief` | 不触发生成 |
| 趋势证据 | 文章详情或原文 | 站内文章当前标签页；原文新标签页 |
| 当前文章追问 | 文章内问答区 | URL 可使用 `#ask` 定位 |
| RAG 引用 | 文章详情证据位置 | 携带 `evidence` 参数或锚点；返回保留问题与回答 |
| “定制我的关注” | `/profile` | 保存后进入 `/?view=for-you` |
| 产品介绍 CTA | 首页、问答、Eval、GitHub | GitHub 新标签；站内当前标签 |
...
2. 确认首页、文章详情、情报问答和产品介绍 PC reference；
...
| 文章详情 | 中文整理、核心判断、影响说明 | 原文、`Evidence`、`ContentVersion` | Python 提取证据；DeepSeek 结构化生成 | 内容入库时 | 展示原始信息并说明摘要不可用 |
...
| 数据状态 | 更新时间、来源和能力状态 | `PipelineRun`、`RuntimeConfig` | Python/Worker 脱敏汇总 | 任务结束或状态变化 | 标明状态本身的最后更新时间 |
| 产品介绍 | 产品文案、架构、截图、实际指标 | Git 仓库文档、实际页面、`EvalRun` | 版本化静态内容 | 发布时 | 规划能力与实际结果分开标注 |
...
- [ ] 8 个核心公开页面均可通过定义入口访问。
- [ ] 顶部导航、画像入口、Eval/状态入口符合第 7.2 节。
- [ ] 首页全部动态、为你推荐、搜索和筛选状态可通过 URL 恢复。
- [ ] 从文章返回首页时恢复筛选、页码和滚动位置。
- [ ] 趋势简报、RAG 引用可以进入对应文章或原始来源。
- [ ] 页面加载、空、失败、历史快照和 AI 关闭状态完整。
- [ ] 页面字段可以对应到第 17.2 节的数据来源和降级策略。
- [ ] 访客端不出现“生成趋势简报”操作。
- [ ] 四张活动 reference 是前端视觉验收基准，历史方案不参与判断。
...
| 为你推荐 | 首页 `view=for-you` 子视图 | 保留同一信息流和 URL 状态管理 |
...
1. 以[视觉设计基准](../design/视觉设计基准.md)、[Design Tokens](../design/设计令牌-Design-Tokens.md)、[组件清单](../design/组件清单.md)、[响应式重排方案](../design/响应式重排方案.md)和四张活动 reference 作为前端依据；
2. 初始化个人 Git 仓库、Vue、Cloudflare Worker、Python 和测试骨架；
3. 使用示例数据完成首页、详情、问答和产品介绍页，并对 6 个固定视口做截图回归；
4. 接入 Python 内容管道、画像、RAG 和 Eval；
5. 按[项目实施计划与进度跟踪](../docs/项目实施计划与进度跟踪.md)持续更新状态。
