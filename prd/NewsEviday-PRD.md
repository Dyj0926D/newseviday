...
| 文档版本   | v1.2.7                                                               |
| 文档状态   | P6.5 对外文案与架构口径校准完成；P7 持久额度、预算和发布 Gate 待实施 |
| 更新日期   | 2026-08-02                                                           |
...
| 产品与技术页       | 从产品和技术两个角度展示定位、能力、架构、RAG 与评测     |
...
| Must   | 产品与技术页，展示产品思路、亮点、技术架构、RAG 和评测                      |
| Must   | 响应式布局与跨设备迁移文档                                                  |
...
| -------------- | --------------- | ------ | ------------------------------------------------ | ------------------------------ | ---------------------------------- |
| `/`            | 首页 / 最新情报 | 核心版 | 全部动态、为你推荐、搜索和筛选                   | Logo、产品与技术 CTA、直接访问 | 情报详情、简报、问答、关注偏好     |
| `/article/:id` | 情报详情        | 核心版 | 中文整理、原始信息、证据、相关内容和当前文章追问 | 最新情报、简报、RAG 引用       | 原文、相关情报、问答               |
| `/ask`         | 证据问答        | 核心版 | 最近 30 天跨文章 RAG                             | 顶部导航、产品与技术           | 引用证据、情报详情、原文           |
| `/brief`       | 7 日趋势简报    | 核心版 | 趋势、变化依据、来源和观察问题                   | 顶部导航、首页信号概览         | 文章、原文、问答                   |
| `/profile`     | 关注偏好        | 核心版 | 输入、智能整理、确认、编辑、导入和导出           | “设置关注偏好”、头像菜单       | 为你推荐、首页                     |
| `/eval`        | 质量评测        | 核心版 | 指标、测试集、版本、门槛和失败案例               | 产品与技术、页脚、问答 Trace   | 问答、产品与技术                   |
| `/status`      | 更新状态        | 核心版 | 更新时间、来源状态、可用能力和已知问题           | 产品与技术、页脚、快照提示     | 首页、产品与技术                   |
| `/product`     | 产品与技术      | 核心版 | 产品思路、能力、架构、RAG、评测、成本和边界      | 顶部导航、页脚                 | 最新情报、问答、质量评测、更新状态 |
...
| 产品与技术              | `/product`           | 同时服务普通用户和面试官                                   |
| 搜索                    | `/?q={keyword}`      | 从任意页面提交后进入首页搜索结果状态                       |
| 设置关注偏好 / 头像菜单 | `/profile`           | 未设置偏好时显示引导；已设置时显示编辑入口                 |

质量评测与更新状态不占用顶部主导航，通过产品与技术页、页脚和相关状态入口访问。私有 GitHub、PRD 和内部进度文档不进入公开导航。
...
- 工程术语使用中文产品名称承接；RAG、Embedding、Trace 等技术词只在产品与技术页和质量评测页解释。
...
| 产品与技术 | `design/reference/视觉基准/04-产品介绍-PC-reference-v1.png` |
...
主要入口：Logo、顶部“最新情报/为你推荐”、产品与技术页“浏览最新情报”、站外分享链接。
...
主要入口：产品与技术、页脚、证据问答“查看完整 Trace”。

主要去向：证据问答、失败案例对应文章、产品与技术的技术章节。
...
路由为 `/product`，导航名称为“产品与技术”。该页面同时服务普通用户和面试官。
...
| 产品与技术 CTA     | 最新情报、问答、质量评测、更新状态 | 站内当前标签                                                  |
...
2. 确认首页、情报详情、证据问答和产品与技术 PC reference；
...
| 产品与技术   | 产品文案、架构、截图、实际指标   | Git 仓库文档、实际页面、`EvalRun`         | 版本化静态内容                          | 发布时               | 目标与实际结果分开标注             |
...
| P3 静态产品    | 完成最新情报、详情和产品与技术页                   | 响应式前端                                           |
...
| 产品与技术   | 独立 `/product` 页面                              | 同时展示产品能力和技术深度                                    |
...
1. P7 接入持久单 IP 限流、全站额度、并发保护和预算台账；
2. 在生产语料和人工复核黄金集上重新运行检索与引用质量评测；
3. 通过 Gate 后执行一次受控 DeepSeek 烟雾测试，再决定是否短时开放证据问答；
4. 完成 Cloudflare 与 EdgeOne Makers 的大陆多网络访问记录；
5. 在第二台设备完成仓库、环境、配置和部署迁移演练；
6. 按[项目实施计划与进度跟踪](../docs/项目实施计划与进度跟踪.md)持续更新状态。
