...
服务端在模型调用前执行：功能开关、配置完整性、请求体校验、Turnstile、预算预留、并发租约、匿名日额度、问题路由、最多两轮检索和证据充分性门禁。证据不足时释放预算并返还额度；模型调用后按 Usage 或保守预留额结算。公开环境继续保持 RAG 关闭，等待人工黄金集、回答引用和线上延迟 Gate。
