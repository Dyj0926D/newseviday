...
| Prompt Injection | 外部内容包装为 untrusted evidence；系统提示声明其无指令权 | RAG 上线前增加注入攻击黄金集 |
...
当前代码已经具备：功能开关、公开限额、DeepSeek 可用性检查、超时/取消、错误分类、Token/估算成本接口。匿名计数与预算台账只有接口，尚未绑定 KV/D1，因此问答公开路由保持关闭。
