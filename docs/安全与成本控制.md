...
| `DEEPSEEK_INPUT_CNY_PER_MILLION`  |                   4 |      0–100000 | 按美元峰时价折算并向上取整的缓存未命中输入预算价   |
| `DEEPSEEK_OUTPUT_CNY_PER_MILLION` |                  12 |      0–100000 | 按美元峰时价折算并向上取整的输出预算价             |
| `RAG_MIN_SCORE`                   |                 1.5 |         0–100 | BM25 检索分数安全下限；仍会检查范围、时间和必需证据 |
| `RAG_MAX_CONTEXT_CHARS`           |                8000 |    1000–20000 | 注入模型的证据字符预算                             |
| `RAG_RETRIEVAL_MODE`              |        `chunk_bm25` | 枚举值          | 低成本分块主路径；可回滚为 `article_dense`         |
...
- 周报生成使用独立 `--usage-report` 记录实际请求数、成功生成调用、缓存命中、输入/输出 Token、Usage 完整性、失败回退类型和费用估算；周报工作流显式注入同一组单价变量，并在 Actions Summary 与文章整理费用分开展示；
