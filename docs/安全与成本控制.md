...
| 模型费用超限 | `AI_ENABLED=false`；D1 预算预留/结算；月预算 35 元、硬上限 50 元；默认不重试 | 首次真实调用前确认模型 ID 与 Token 单价 |
...
当前代码已经具备：功能开关、Turnstile 服务端校验、D1 持久限额、并发租约、预算预留/结算、DeepSeek 可用性检查、检索阈值拒答、超时/取消、错误分类、匿名 RAG Trace 和 Token/估算成本接口。保护存储或配置不可用时直接返回 `guardrails_unavailable`，不会降级为无保护调用。远程 D1 已创建并完成迁移；AI 与 RAG 仍保持关闭，等待真实模型烟雾测试与 RAG 质量 Gate。
...
| `GLOBAL_DAILY_GENERATIONS` | 20 | 0–10000 | 全站每日生成尝试上限 |
| `SOFT_BUDGET_DAILY_GENERATIONS` | 10 | 0–10000 | 到达软预算后的全站日上限 |
| `MAX_CONCURRENT_GENERATIONS` | 2 | 1–20 | 全站同时运行的生成请求 |
...
| `DEEPSEEK_MODEL` | `deepseek-v4-pro` | 官方 model ID | 2026-08-02 核对的 V4 Pro 标识；调用前仍需复核 |
| `DEEPSEEK_THINKING_ENABLED` | `false` | true/false | MVP 默认非思考模式，控制延迟和输出 Token |
| `DEEPSEEK_INPUT_CNY_PER_MILLION` | 3 | 0–100000 | 当期最高输入价（缓存未命中价）；调用前复核 |
| `DEEPSEEK_OUTPUT_CNY_PER_MILLION` | 6 | 0–100000 | 当期输出价；调用前复核 |
...
`DEEPSEEK_MODEL` 必须使用供应商控制台当时给出的准确 model ID。2026-08-02 已按官方文档核对为 `deepseek-v4-pro`，缓存未命中输入价 3 元/百万 Token、输出价 6 元/百万 Token。价格可能调整，真实调用前仍需复核。V4 默认开启思考模式，本项目对摘要、画像和证据问答显式使用非思考模式，避免无必要的思考 Token 和延迟。
...
- 到达 35 元软预算后，全站日额度自动收紧为 10；到达 50 元硬上限后状态变为 `budget-paused`；
