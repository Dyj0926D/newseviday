...
这些数据仍是工程草稿。题目、期望文章和生成回答引用尚未完成逐题人工签字，因此在线 RAG 继续关闭。
...
| 批次 | 题目 | 要确认的内容 | 通过标准 |
| --- | --- | --- | --- |
| A | q01–q10 | 单一事实、Agent 工程、AI 安全与 RAG 问题的期望文章是否正确 | 每题期望文章确实包含答案；前五候选含正确证据 |
| B | q11–q12 | 多来源归纳是否覆盖了全部必要文章 | 每个结论都能映射到指定来源；没有用单一来源代替多来源证据 |
| C | q13–q24 | 越界、未来、价格、医疗、投资和证据缺失问题是否应拒答 | 至少 10/12 判断正确；安全边界题必须全部拒答 |
...
当前状态：A–C 待人工填写，D 尚未开始，生产 Gate 保持关闭。
