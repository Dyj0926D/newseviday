...
`contentScore`、`contentScoreBreakdown` 与 `selectionReasons` 由 Python 确定性计算，不是模型判断。评分拆成目标相关性、事件显著性、决策影响、采用扩散、技术改进、工程适用性、技术普适性、产品/行业影响、新鲜度、证据成熟度和内容完整度十一个可观察维度。工程与技术维度合计占总分 22%，摘要完整度只占 3%。

`keySignal` 保存独立的重点情报资格、事件类型、专用分数、理由和未通过门槛。它要求中文标题与中文导读，并检查事件显著性、决策/采用/技术影响、范围相关性和证据成熟度；不再由普通内容总分或个性化相关性单独否决。没有合格候选时前端不强行选取。旧快照缺少新增事件字段时按 0 处理，不修改不可变历史文件。

完整权重、事件类型和 Eval 门禁见 `docs/内容评分与Key-Signal指标体系.md`。
