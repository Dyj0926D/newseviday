...
| 项目     | 当前基线                                                           |
| -------- | ------------------------------------------------------------------ |
| 架构版本 | v0.5 / P6.5 对外口径与架构校准                                     |
| 日期     | 2026-08-02                                                         |
| 状态     | P4–P6 工程基线完成；公开内容稳定可用，在线生成继续受发布 Gate 控制 |
...
浏览器只访问 Worker，不直接接触 DeepSeek Key。当前 Worker 已处理能力开关、请求校验、超时、错误语义、文章级候选召回和流式输出；持久限流、并发额度和预算台账尚未接入，因此在线生成保持关闭。Key 是否存在不等于能力已经开放，也不会自动产生 Token 费用。
...
- 请求体与超时已有强制边界；每日次数和预算目前只有配置与接口，持久执行器待 P7 接入；
- 默认重试 0 次，避免生成请求重复计费；
...
| 持久限流与预算  | 已有接口，尚未进入请求执行链                         | 列为 P7 公开生成前置 Gate                          |
