...
| Python Pipeline | `pipeline`                          | Feed 解析、清洗、去重、价值排序、中文整理、周报与快照发布 | 9 个来源真实运行；日更与周报均有发布 Gate |
...
公共趋势简报使用独立 `weekly-brief.yml`。任务在北京时间每周六 10:00 读取上周六至本周五的中文就绪文章，DeepSeek 单次输出 1–3 个趋势，每节必须引用 2–5 条证据且覆盖至少两个来源。证据 ID 会回查快照中的 `Evidence`，未知证据、单一来源或字段不完整都会拒绝发布。日更任务会携带最近成功简报，避免新内容快照把周报清空。
