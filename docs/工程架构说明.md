...
内容评分不调用模型。Python 使用八个可观察维度计算 `contentScore`，其中目标相关性占 30%，技术改进、工程适用性和技术普适性合计占 25%。论文摘要完整只贡献 5%，避免结构完整的论文天然占优。arXiv 每日上限为 6 篇。Vue 的“最新情报”默认取近 30 天中文就绪内容并按发布时间倒序，再执行同源连续性修正；“为你推荐”才使用画像相关度和内容分。Key Signal 另设资格门禁，未达标时首页不展示重点位。

AI 结构化整理每次最多处理 5 篇。优先队列先处理“只缺中文即可成为 Key Signal”的候选，再处理总分不低于 0.60 的外文内容和首页前 8 条外文内容，并对重复来源施加降权。中文原文具备中文标题和来源摘要时直接通过中文展示门禁；外文内容只有同时生成中文标题和中文导读后才显示“已整理”。
