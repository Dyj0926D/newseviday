...
内容评分不调用模型。Python 使用十一项可观察维度计算 `contentScore`，并将事件显著性、决策影响和采用扩散从画像相关性中拆出。技术改进、工程适用性和技术普适性合计占 22%，论文摘要完整只贡献 3%。arXiv 每日上限为 6 篇。Vue 的“最新情报”默认取近 30 天中文就绪内容并按发布时间倒序，再执行同源连续性修正；“为你推荐”才使用画像相关度和内容分。Key Signal 按变化事件、证据和可行动性设置独立门禁，未达标时首页不展示重点位。

`eval-key-signal` 使用跨事件黄金集计算资格 Precision/Recall/F1、高显著性候选 Precision/Recall/F1 和事件类型准确率。该评测在每日内容工作流内运行，失败时终止候选发布。

AI 结构化整理每天发放 5 次基础额度，未使用额度写入私有台账并最多结转 10 次，单次任务仍以 10 次为硬上限。优先队列先处理“只缺中文即可成为 Key Signal”的候选，再处理高显著性变化事件、四个个人重点主题和可信来源的新鲜高价值内容，最后处理达到普通付费门槛的外文内容，并对重复来源施加降权。中文原文具备中文标题和来源摘要时直接通过中文展示门禁；外文内容只有同时生成中文标题和中文导读后才显示“已整理”。
