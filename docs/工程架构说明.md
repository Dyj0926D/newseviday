...
AI 结构化整理每次最多处理 5 篇。优先队列先处理“只缺中文即可成为 Key Signal”的候选，再处理高显著性变化事件，然后处理总分和主题相关性达到普通付费门槛的外文内容，并对重复来源施加降权。中文原文具备中文标题和来源摘要时直接通过中文展示门禁；外文内容只有同时生成中文标题和中文导读后才显示“已整理”。
...
任何检索策略升级都应带 `datasetVersion` 和 `EvalRun`，未过 gate 不替换生产默认值。固定生产语料已建立 24 题试运行集；证据门禁无答案识别达到 1.0，旧单阈值基线仍为 0.25。黄金题和回答引用尚未人工复核，因此 Gate 为 fail，在线 RAG 保持关闭。
