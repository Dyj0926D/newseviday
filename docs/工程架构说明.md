...
| Python Pipeline | `pipeline`                          | Feed 解析、清洗、去重、价值排序、快照发布 | 9 个来源真实运行；首批增强 3 篇   |
...
内容排序不调用模型。Python 使用八个可观察维度计算 `contentScore`，其中目标相关性占 30%，技术改进、工程适用性和技术普适性合计占 25%。论文摘要完整只贡献 5%，避免结构完整的论文天然占优。arXiv 每日上限为 6 篇；Vue 在保留分数顺序的基础上执行首屏来源重排，同一来源前 8 条最多 3 条，并阻止连续 3 条。Key Signal 另设资格门禁，未达标时首页不展示重点位。
