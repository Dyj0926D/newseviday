...
| 项目     | 当前基线                                                           |
| -------- | ------------------------------------------------------------------ |
| 架构版本 | v0.9 / 内容质量与有限步骤 Agentic RAG                               |
| 日期     | 2026-08-06                                                         |
| 状态     | 生产快照、质量审计与 Agentic Eval 已落地；在线 AI/RAG 继续受 Gate 约束 |
...
flowchart TB
    ...
    Sources --> Fetch --> Norm --> Exact --> Fuzzy --> Select --> Key --> Snap
    Snap --> Vue
    Vue --> Worker
    Vue -. "API 失败" .-> Snap
    Snap --> Edge
...
| Python Pipeline | `pipeline`                          | Feed 解析、清洗、去重、价值排序、中文整理、周报与快照发布 | 15 个来源配置；日更与周报均有发布 Gate |
...
内容评分不调用模型。Python 使用十一项可观察维度计算 `contentScore`，并将事件显著性、决策影响和采用扩散从画像相关性中拆出。技术改进、工程适用性和技术普适性合计占 22%，论文摘要完整只贡献 3%。arXiv 每日上限为 6 篇。Vue 的“最新情报”默认取近 30 天中文就绪内容并按发布时间倒序，再执行同源连续性修正；“为你推荐”才使用画像相关度和内容分。Key Signal 按变化事件、证据和可行动性设置独立门禁，观察窗为 5 天；前端优先选择最新合格事件，使新候选在日更发布后直接替换旧重点位。

`eval-key-signal` 使用跨事件黄金集计算资格 Precision/Recall/F1、高显著性候选 Precision/Recall/F1 和事件类型准确率。该评测在每日内容工作流内运行，失败时终止候选发布。

AI 结构化整理每天发放 5 次基础额度，未使用额度写入私有台账并最多结转 10 次，单次任务仍以 10 次为硬上限。优先队列先处理“只缺中文即可成为 Key Signal”的候选，再处理高显著性变化事件、四个个人重点主题、可信来源的新鲜高价值内容和高证据成熟度的通用工程指导，最后处理达到普通付费门槛的外文内容，并对重复来源施加降权。中文原文具备中文标题和来源摘要时直接通过中文展示门禁；外文内容只有同时生成中文标题和中文导读后才显示“已整理”。

日更使用北京时间 09:17 主调度和 11:47、15:17、19:47 三次恢复调度；成功 Artifact 会阻止同日后续任务重复安装依赖或调用模型。独立监测工作流每 6 小时检查线上快照时间，超过 26 小时只创建一个告警 Issue，快照恢复后自动关闭。

滚动库存为 Data Agent、数据平台、统一语义和智能湖仓保留近 7 天高相关候选。内容质量报告同时计算 24/48 小时中文可见数量、最新中文可见时间、中文可见主题缺口和新增文章数；这些信号进入 Actions Summary，便于区分“任务成功”和“网站确实出现新内容”。

公共趋势简报使用独立 `weekly-brief.yml`。周六由该工作流独占内容采集：先实时抓取全部来源、合并滚动库存、复用缓存并整理高价值外文，再筛选上周六 09:00 至本周六 09:00 的中文就绪文章。DeepSeek 单次输出 1–3 个趋势，每节必须引用 2–5 条证据且覆盖至少两个来源。每节还需包含一手来源，或至少两个独立二手来源；独立作者观点不能单独支撑结论。证据 ID 会回查快照中的 `Evidence`，未知证据、证据层级不足、单一来源或字段不完整都会拒绝发布。周报候选同时携带最新内容快照，日更任务则保留最近成功简报。
...
3. Python 离线链路与 Worker 默认使用 `chunk_bm25`，`article_dense` 保留为配置化回滚路径；
4. BGE-M3 适配器和 Vectorize NDJSON 只属于生产候选，外部索引、binding、维度与云端时延未验证前不替换在线默认值；
5. `bounded_v1` 已实现固定路由、最多两个检索表达、证据缺口检查和提前停止；`hybrid_rerank` 仍留在增强实验范围；
6. 外部文本统一标记为不可信证据，不能改变系统指令；证据不足时明确拒答。

当前线上 Trace 写入 Cloudflare 日志；离线 Eval Harness 已把 retriever input、ranked candidates、injected context、answer、逐句 citation、停止原因和人工标签写入版本化评测包。返回引用仍只表示“证据已注入回答上下文”，引用是否支持事实句必须经过人工忠实度复核。
...
任何检索策略升级都必须带 `datasetVersion` 和 `EvalRun`。固定生产语料的 24 题内部集已人工签字，`chunk_bm25` 达到 Recall@5/10 1.0、MRR 0.9167；MultiHop-RAG 2,255 道可回答问题全量评测达到 Recall@5 0.7394、MRR 0.8418。`eval-rag-answers` 可分批缓存真实回答并导出逐句引用标签；生成回答人工 Gate 尚未完成，因此在线 RAG 保持关闭。
...
- 15 个海内外 Atom/RSS/HTML 来源，覆盖官方、论文、研究机构、专业媒体和独立作者；包含来源级标题解析、正文清洗、精确/模糊去重、8 主题筛选、确定性价值排序、内容配额、持久 Trace、原子快照；
- 单篇一次 AI 丰富、术语表、内容哈希缓存、单次最多 10 次模型调用的硬上限与画像确认流程；
- 离线 Chunking、hashing dense、article fallback、Vectorize 导出、30 题 Demo 集和 24 题生产试运行集；
- `bounded_v1` 问题路由、最多两轮检索、证据充分性判断、提前停止与增强 Trace；
- 内容价值分、来源/主题覆盖质量报告、潜在多来源事件检测和公开历史索引；
...
| 项目            | 结论                                                 | 处理                                               |
| --------------- | ---------------------------------------------------- | -------------------------------------------------- |
| 线上/离线口径   | 原线上文章级、离线分块级不一致                       | 已统一为 `chunk_bm25`，保留 `article_dense` 回滚   |
...
| 引用质量        | 已返回候选引用，尚不能证明回答忠实使用               | 已增加逐句引用、正确性、完整性人工 Gate            |
| 多来源合并      | 精确去重会只保留一个主来源，旁证关系未建模           | 后续增加 `alternateEvidence`，不在本轮修改数据契约 |
| 生产拒答质量    | 单一低分阈值无答案识别仅 0.25                         | 增加范围、时间和必需证据门禁；工程草稿达 1.0，待人工复核 |
