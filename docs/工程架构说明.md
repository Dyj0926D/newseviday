...
公共趋势简报使用独立 `weekly-brief.yml`。任务在每周一 12:30 读取上一完整自然周的中文就绪文章，DeepSeek 单次输出 1–3 个趋势，每节必须引用 2–5 条证据且覆盖至少两个来源。证据 ID 会回查快照中的 `Evidence`，未知证据、单一来源或字段不完整都会拒绝发布。日更任务会携带最近成功简报，避免新内容快照把周报清空。
