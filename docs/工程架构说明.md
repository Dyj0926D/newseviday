...
任何检索策略升级都应带 `datasetVersion` 和 `EvalRun`，未过 gate 不替换生产默认值。固定生产语料已建立 24 题试运行集；证据门禁无答案识别达到 1.0，旧单阈值基线仍为 0.25。`eval-rag --review-packet` 可导出逐题路由、前五个 Chunk、证据判断和空白人工标签，复核流程见 [RAG 人工复核清单](./RAG人工复核清单.md)。黄金题和回答引用尚未人工签字，因此 Gate 为 fail，在线 RAG 保持关闭。
