...
| 项目     | 当前基线                                                           |
| -------- | ------------------------------------------------------------------ |
| 架构版本 | v0.6 / P7 持久保护与部署 Gate                                       |
| 日期     | 2026-08-02                                                         |
| 状态     | P4–P7 工程基线完成；动态生成保护已落地，在线生成继续受质量 Gate 控制 |
...
| Python Pipeline | `pipeline`                          | Feed 解析、清洗、去重、主题筛选、快照发布 | 9 个来源真实运行通过，0 次模型调用 |
...
浏览器只访问 Worker，不直接接触 DeepSeek Key。生成请求先经过 Turnstile 服务端校验、D1 月预算预留、并发租约、单 IP/全站日额度，再进入画像增强或 RAG。模型返回 Usage 时按配置单价结算；缺少 Usage 或上游失败时按预留额保守入账；证据不足且未调用模型时释放预算并返还日额度。AI 与 RAG 总开关仍关闭，因此当前不会产生模型费用。
...
当前 Trace 写入 Cloudflare 日志，尚未进入持久评测库。返回引用表示“证据已注入回答上下文”，不能自动证明每个事实都正确使用了引用；引用覆盖率和忠实度需要 P7/P8 人工评测后才形成发布结论。
...
- 9 个真实官方 Atom/RSS/HTML 来源、来源级标题解析、正文清洗、精确/模糊去重、8 主题筛选、内容配额、持久 Trace、原子快照；
- 单篇一次 AI 丰富、术语表、内容哈希缓存与画像确认流程；
- 离线 Chunking、hashing dense、article fallback、Vectorize 导出和 30 题 Eval Runner；
...
| 引用质量        | 已返回候选引用，尚不能证明回答忠实使用               | P7/P8 增加引用覆盖与人工复核                       |
