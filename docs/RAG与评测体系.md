...
生成回答采用分批缓存：每次最多 5 次 DeepSeek 调用，重复运行复用已生成答案。缓存同时写入 GitHub Actions Cache 和保留 30 天的私有评测 Artifact；即使 Actions Cache 因长期未访问被回收，后续批次仍可从最近 Artifact 恢复。每个可回答问题记录完整 Trace，并按以下条件发布：
